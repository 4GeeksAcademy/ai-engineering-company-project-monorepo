...
- **Archivos permitidos y obligatorios:** `index.html` (Landing), `application.html` (Formulario), `validation.js` (Lógica), `styles.css` (SOLO si Tailwind no es suficiente, aunque la prioridad es cero CSS personalizado).
...
**Contexto:** Interfaz administrativa (`/admin`) donde los consultores visualizan el ranking automatizado de candidatos para una oferta laboral, evaluados previamente por el ecosistema de IA.
...
- **Diseño Condicional (Tailwind CSS v4):**
  ...
  - Las tarjetas deben incluir un componente desplegable (Accordion/Details) manejado con Vanilla JS para mostrar el `razonamiento_corto` sin saturar la vista principal.
